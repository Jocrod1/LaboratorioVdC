...
namespace Interfaz
{
    public partial class Sedimentos : Form
    {
        ...
        public Sedimentos()
        {
            ...
            this.toolTip1.SetToolTip(this.btnNuevo, "Agregar Sedimento");
            this.toolTip1.SetToolTip(this.txtNota, "Nota de la Sedimento");
        }

        private void Sedimentos_Load(object sender, EventArgs e)
        {
            this.Mostrar();
            this.Deshabilitar();
            this.Botones();
        ...
        private void OcultarColumnas()
        {
            ...
            //renombrar las otras 
            this.dataListado.Columns[1].HeaderText = "Nota Sedimentos";
        ...
        private void EliminarItems()
        {
            ...
            try
            {
                ...
                {
                    Opcion = MessageBox.Show("¿Realmente desea eliminar los " + NumeroSeleccionado + " registros de Sedimentos?", "Laboratorio Clínico Virgen de Coromoto", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
                else
                {
                    Opcion = MessageBox.Show("¿Realmente desea eliminar el registro del sedimento?", "Laboratorio Clínico Virgen de Coromoto", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow item in this.dataListado.SelectedRows)
                    {
                        Rpta = MSedimentos.Eliminar(Convert.ToInt32(item.Cells["ID"].Value));
                    }
                    ...
                    if (Rpta.Equals("OK"))
                    {
                        if (NumeroSeleccionado > 1)
                        {
                            this.MensajeOK("Se Eliminaron Correctamente los Registros de Sedimentos");
                        }
                        else
                        {
                            this.MensajeOK("Se Eliminó Correctamente el Registro del Sedimento");
                        }
                    }
                    else
                    {
        ...
        void AnularItems()
        {
            ...
            try
            {
                ...
                if (NumeroSeleccionado > 1)
                {
                    Opcion = MessageBox.Show("¿Realmente desea anular los " + NumeroSeleccionado + " registros de Sedimentos?", "Laboratorio Clínico Virgen de Coromoto", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
                else
                {
                    Opcion = MessageBox.Show("¿Realmente desea anular el registro del Sedimento?", "Laboratorio Clínico Virgen de Coromoto", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow item in this.dataListado.SelectedRows)
                    {
                        Rpta = MSedimentos.Anular(Convert.ToInt32(item.Cells["ID"].Value));
                    }
                    ...
                    if (Rpta.Equals("OK"))
                    {
                        if (NumeroSeleccionado > 1)
                        {
                            this.MensajeOK("Se Anularon Correctamente los Registros de Sedimentos");
                        }
                        else
                        {
                            this.MensajeOK("Se Anuló Correctamente el Registro del Sedimentos");
                        }
                    }
                    else
                    {
        ...
        private void Guardar()
        {
            try
            {
                ...
                if (this.IsNuevo)
                {
                    ...
                    Rpta = MSedimentos.Insertar(this.txtNota.Text);
                }
                else
                {
                    //Vamos a modificar un Paciente
                    Rpta = MSedimentos.Editar(ID, this.txtNota.Text);
                }
        ...
        private bool validar()
        {
            ...
            {
                error = false;
                errorProvider1.SetError(txtNota, "Agrega un Sedimento");
            }
            return error;
        }
        ...
        {
            dataListado.DataSource = MSedimentos.Mostrar(txtBuscar.Text);
