...
namespace Interfaz
{
    public partial class Antibioticos : Form
    {
        ...
        public Antibioticos()
        {
            ...
            this.toolTip1.SetToolTip(this.btnNuevo, "Agregar antibiotico");
            this.toolTip1.SetToolTip(this.txtNombre, "Nombre del Antibiotico");
        }


        private void Antibioticos_Load(object sender, EventArgs e)
        {
            this.Mostrar();
            this.Deshabilitar();
            this.Botones();
        ...
        private void OcultarColumnas()
        {
            ...
            //renombrar las otras 
            this.dataListado.Columns[1].HeaderText = "Nombre Antibiotico";
        ...
        private void EliminarItems()
        {
            ...
            try
            {
                ...
                {
                    Opcion = MessageBox.Show("¿Realmente desea eliminar los " + NumeroSeleccionado + " registros de Antibioticos?", "Laboratorio Clínico Virgen de Coromoto", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
                else
                {
                    Opcion = MessageBox.Show("¿Realmente desea eliminar el registro del antibiotico?", "Laboratorio Clínico Virgen de Coromoto", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow item in this.dataListado.SelectedRows)
                    {
                        Rpta = MAntibioticos.Eliminar(Convert.ToInt32(item.Cells["ID"].Value));
                    }
                    ...
                    if (Rpta.Equals("OK"))
                    {
                        if (NumeroSeleccionado > 1)
                        {
                            this.MensajeOK("Se Eliminaron Correctamente los Registros de antibioticos");
                        }
                        else
                        {
                            this.MensajeOK("Se Eliminó Correctamente el Registro del antibiotico");
                        }
                    }
                    else
                    {
        ...
        void AnularItems()
        {
            ...
            try
            {
                ...
                if (NumeroSeleccionado > 1)
                {
                    Opcion = MessageBox.Show("¿Realmente desea anular los " + NumeroSeleccionado + " registros de antibioticos?", "Laboratorio Clínico Virgen de Coromoto", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
                else
                {
                    Opcion = MessageBox.Show("¿Realmente desea anular el registro del antibiotico?", "Laboratorio Clínico Virgen de Coromoto", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow item in this.dataListado.SelectedRows)
                    {
                        Rpta = MAntibioticos.Anular(Convert.ToInt32(item.Cells["ID"].Value));
                    }
                    ...
                    if (Rpta.Equals("OK"))
                    {
                        if (NumeroSeleccionado > 1)
                        {
                            this.MensajeOK("Se Anularon Correctamente los Registros de antibioticos");
                        }
                        else
                        {
                            this.MensajeOK("Se Anuló Correctamente el Registro del antibiotico");
                        }
                    }
                    else
                    {
        ...
        private void Guardar()
        {
            try
            {
                ...
                if (this.IsNuevo)
                {
                    ...
                    Rpta = MAntibioticos.Insertar(this.txtNombre.Text);
                }
                else
                {
                    //Vamos a modificar un Paciente
                    Rpta = MAntibioticos.Editar(ID, this.txtNombre.Text);
                }
        ...
        {
            dataListado.DataSource = MAntibioticos.Mostrar(txtBuscar.Text);
